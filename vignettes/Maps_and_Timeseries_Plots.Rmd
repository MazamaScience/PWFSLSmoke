...
The `monitor_map()` and `monitor_leaflet()` plots show us pretty much the same information, except that the leaflet plot allows you to get monitor-specific metadata by clicking on a monitor.
...
```{r googleMap, fig.width=5, fig.height=5, eval=FALSE, echo=FALSE}
...
<!--
From this map we can see that the monitor in the Colville reservation also registered a daily AQI level of "hazardous"
...
in a new "ESRI" map of the August daily average maxima with fire icons at fire 
locations:
