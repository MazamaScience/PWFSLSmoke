...
#'   logger.trace("Replacing 0,0 locations with NA,NA for IDs: %s", badLocationIDs)
#'   df$longitude[badLocationMask] <- NA
#'   df$latitude[badLocationMask] <- NA
#'
#'   # Pull out unique monitors
#'   dfUnique <- df[!duplicated(df$monitorID),]
#'   logger.trace("Dataframe contains %d unique monitorID(s)", nrow(dfUnique))
#'
...
#'
#'   logger.trace("Created 'meta' dataframe with %d rows and %d columns", nrow(metaDF), ncol(metaDF))
#'
#'   # ----- Data Reshaping ------------------------------------------------------
#'
#'   logger.trace("Reshaping OpenAQ sites metadata ...")
#'
...
#'     if ( length(repIDs) >0 ) {
#'
#'       logger.trace("There are %i monitor(s)' lat/lon will be averaged", length(repIDs))
#'
#'       for (id in repIDs ) {
#'
