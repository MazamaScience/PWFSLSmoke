...
wrcc_identifyMonitorType <- function(fileString) {
  ...
  ebam_header <- vector('character',3)
  ebam_header[1] <- ":       GMT\t Deg \t Deg \t     \tser #\tug/m3\t Unk \t l/m \tDeg C\t  %  \t Unk \tdeg C\t  %  \t m/s \t Deg \tvolts\t"                                   
  ebam_header[2] <- ": Date/Time\t  GPS  \t  GPS  \tType   \tSerial \tConc   \t Misc  \t Ave.  \t Av Air\t  Rel  \t Misc  \tSensor \tSensor \t  Wind \t Wind  \tBattery\tAlarm"
  ebam_header[3] <- ":YYMMDDhhmm\t  Lat. \t  Lon. \t       \tNumber \t RT    \t  #1   \tAir Flw\t  Temp \tHumidty\t  #2   \tInt AT \tInt RH \t  Speed\t Direc \tVoltage\t"     
  ebam_rawNames <- c('DateTime','GPSLat','GPSLon','Type','SerialNumber','ConcRT','Misc1',
                     'AvAirFlw','AvAirTemp','RelHumidity','Misc2','SensorIntAT','SensorIntRH',
                     'WindSpeed','WindDir','BatteryVoltage','Alarm')
  ebam_names <- ebam_rawNames
  ebam_types <- 'cdddcdddddddddddd'
  
  #      Extract  header lines from the incoming fileString     ---------------
  
  # NOTE:  Here is an example of what WRCC ASCII output might look like:
  # NOTE:  
  # NOTE:  [1] " Smoke #11 "                                                                                                                                                
