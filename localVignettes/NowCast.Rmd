...
This vignette documents the basis and functionality of the `monitor_nowcast()` function, which is used to create NowCast data for `ws_monitor` objects. It also covers the `monitor_aqi()` function, which converts raw pm2.5 data into AQI values.

### What is NowCast?

NowCast is an algorithm designed to approximate real-time air quality readings to 24-hour averages, upon which health advisory levels are based.

When air quality readings are relatively stable, NowCast approximates a 12-hour average. When readings are changing rapidly, NowCast more closely captures current conditions.
...
NowCast is calculated as follows.
...
Note that this is equivalent to the following, which is seen and described in some of the literature.
...
$$
\frac{\sum_{i=1}^{12}{w^{i-1}c_i}}{\sum_{i=1}^{12}w^{i-1}}
$$
