...
```{r monthly_maps, fig.width = 8, fig.height=24.5, cache = TRUE}
...
monitorPlot_timeseries(yrekaNowcast, type = "l", add = TRUE, col = adjustcolor('royalblue', alpha.f = .7), 
                       tlim = c(20170121, 20170830), lwd = 2, title = FALSE)
legend('topright', legend = c("hourly PM2.5", "nowcast", "3-hour rolling mean"), pch = c(16, NA, NA), 
       lty = c(NA, 1, 1), col = c('gray20', adjustcolor('royalblue', alpha.f = .7), 'black'), lwd = c(NA, 2, 1))
```
...
monitorPlot_dailyBarplot(yrekaWeek)
```
...
maps::map("county", "California", ylim = c(40.9, 42.1), xlim = c(-124.3, -121.1))
addMarker(sisk2017$meta$longitude, sisk2017$meta$latitude, expansion = .5, color = "yellow")
