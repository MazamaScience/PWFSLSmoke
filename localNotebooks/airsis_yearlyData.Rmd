...
Create some maps and plots to get an idea about what the data looks like. MonitorLeaflet creates an interactive map with monitors plotted as points and colored to correspond to the AQI level of the maximum PM2.5 value in the data. 
...
Now that we've seen when the data was, let's look at where it is. There are several different options for plotting monitors from a ws_monitor object. `monitorEsriMap()` and `monitorGoogleMap()` will result in very similar images. Each will plot points for each monitor with AQI colors indicating the maximum PM2.5 value over a map image, sourced either from ESRI or GoogleMaps. `monitorEsriMap` uses R's base plotting capabilities so it is easier to add points or polygons on top. For either one, there is an option to plot the points on a map object which already exists in memory, retrieved with either `esriMap_getMap()` or `RgoogleMaps::getMap()`. Retrieving the map image from the server will probably take the longest time in plotting the map, so use this option when plotting many maps with the same extent. 

The other option for plotting monitor locations is `monitorMap()` which plots the points over polygons instead of an image. 
