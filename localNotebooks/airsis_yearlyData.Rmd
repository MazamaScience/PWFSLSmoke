...
```{r daily_barplot}
monitorPlot_dailyBarplot(monitor_subset(sisk2017, monitorID="lon_.122.634_lat_41.727_apcd.1014"), tlim = c(20170821, 20170830))
```

A daily barplot is good for summarizing trends, but it can erase some dramatic highs and lows. A rolling mean plot includes the hourly values as points but draws a rolling-mean line which helps smooth out the data. 

```{r rolling_mean}
monitorPlot_rollingMean(monitor_subset(sisk2017, monitorID="lon_.122.634_lat_41.727_apcd.1014"), tlim = c(20170821, 20170830))
```
